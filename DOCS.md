...
| Know why a new, separate ASR-representation research track opened, what it found (Stages A-C done), and exactly what to do next | `ASR_RESEARCH_TRACK.md` (its end-of-session handoff section specifically) |
...
| **`ASR_RESEARCH_TRACK.md`** | Research/planning + living results, **charter and results record for a separate branch (`asr-research`)** | Opened 2026-08-05 after Track B validation (`VALIDATION.md` §14) found real ASR essentially never preserves the sub-word fragment tokens `sound_repetition` detection depends on. Reframes the core question as "how do we preserve the speech-production information conventional ASR intentionally removes," reviews the literature, lays out architectural directions without committing to one, and sets a phased, evidence-gated research plan (Stages A-E). **As of this session's close, Stages A, B, and C are done** — a real, traced information-loss mechanism (Stage A), a genuine but duration-confound-refuted encoder signal for `sound_repetition` that isn't yet precise enough to ship alone (Stages B/C) — with an end-of-session handoff section (current state, open questions, exact next-session plan) at the end of the file. Explicitly does not reopen `PHASE_3_ARCHITECTURE_REVIEW.md`'s two-stage-architecture conclusion. `main` stays stable throughout; nothing from this track has merged. | Updated as each stage in the plan produces results, on the `asr-research` branch — not on `main` until a stage's evidence is ready to ship. |
